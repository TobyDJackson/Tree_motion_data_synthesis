function [fit_result] = plot_welch_test040820(tree_in,fs_tree,wind_in,fs_wind,...
    ...
    if Ebba==1 % pre-multiply by f as suggested by Ebba
        ...
        if fs_wind>0
            pxx_wind=cat(2,f_wind.*pxx_wind(:,1),f_wind.*pxx_wind(:,2));
        end
    end
    ...
    if PLOT==4
        if fs_wind>0
            ...
            [fit_result_wind, gof] = fit_spline_pxx(log(f_wind),log(pxx_wind(:,1)),1,p_smooth);
            h1=plot( fit_result_wind ); legend off; hold on;
            [fit_result_wind, gof] = fit_spline_pxx(log(f_wind),log(pxx_wind(:,2)),1,p_smooth);
            h2=plot( fit_result_wind ); legend off; hold on;
            set(h1,'color','black','LineWidth',1.5,'LineStyle','-')
            set(h2,'color','black','LineWidth',1.5,'LineStyle','-')
        end
        xlim([log(0.001) log(fs_tree/2)]);
